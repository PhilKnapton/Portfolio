...
In this screenshot, the disaster recovery architecture for Project Pegasus is shown. The diagram illustrates the PHOENIX and LIVERPOOL datacenters connected through VMware vSphere Replication and Site Recovery Manager. In my implementation, the physical ESXi host was used as the recovery site while the primary production environment was hosted on a nested ESXi host in VMware Workstation. This approach allowed more efficient use of available resources while still demonstrating full disaster recovery functionality. Both sites include vCenter Server, vSphere Replication, Site Recovery Manager, and DNS services to coordinate replication, failover, and failback of the Windows File Server VM.
